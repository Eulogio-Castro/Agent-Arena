...
public class WallRun : MonoBehaviour
{
    ...
    NetworkPlayerMovementController movementController;
    NetworkPlayerCameraController cameraController;
    ...
    bool CanWallRun()
    {
        float verticalAxis = movementController.oldInput.y;
    ...
    void Start()
    {
        ...
        movementController = GetComponent<NetworkPlayerMovementController>();
    ...
    {  
        ...
        if(movementController.hasJumped)
        {
            jumping = true;
        }
        ...
        if(isWallRunning)
        {
            ...
            movementController.moveVector += Vector3.down * wallGravityDownForce * Time.deltaTime;
        }
        else
        {   
    ...
    void OnWall(RaycastHit hit){
        ...
        if(d >= -normalizedAngleThreshold && d <= normalizedAngleThreshold)
        {
            ...
            float vertical = movementController.oldInput.y;
            Vector3 alongWall = transform.TransformDirection(Vector3.forward);

            Debug.DrawRay(transform.position, alongWall.normalized * 10, Color.green);
            Debug.DrawRay(transform.position, lastWallNormal * 10, Color.magenta);

            movementController.moveVector = alongWall * vertical * wallSpeedMultiplier;
            isWallRunning = true;
        }
    }
    ...
    public float GetCameraRoll()
    {
        ...
        float cameraAngle = cameraController.transform.eulerAngles.z;
        float targetAngle = 0;
        if(dir != 0)
        {
